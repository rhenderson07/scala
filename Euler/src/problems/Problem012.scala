...
object Problem012 extends Problem with App {
  ...
  lazy val triangles4 = Stream.from(2).scanLeft(1)(_ + _)
  ...
  // second attempt. Uses fast method to count divisors. Runs instantly
  ...
  //println(run)
  val testVal = 100000000
  println(MyMath.divisorCount(testVal))
  println(MyMath.findDivisors(testVal).length)
}